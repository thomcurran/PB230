...
Path analysis is theoretically useful because, unlike GLM, it forces us to specify relationships among all of the variables. This results in models showing causal mechanisms through which independent variables produce both direct and indirect effects on a dependent variable. 

We tested mediation last year, and if you recall, we had to do this with multiple models estimating the a, b, c, and c' paths. Path analysis allows us to test these paths simultaneously and omit any paths we think are redundant (ie., the direct effect)
...
data_var <-
  employee %>%
  select(3:5) # select out only the items
md.pattern(data_var)
```
...
![](https://i.ibb.co/2ZZrdK0/justmodelsch.png)

A couple of addition things to note:

1. Within the mediation models, I label coefficients a, b and c with the astrix (*).

2. You can use ":=" to define a new parameter, such as the indirect effect (ab). Note when you define new parameter with :=, you can use the astrix to multiply values. This is very useful for mediation models becuase the indirect effect is a multiplied by b.
...
![](https://i.ibb.co/YhhRC2X/justmodelout1.png)
...
![](https://i.ibb.co/DRXcQbt/overmodelsch.png)
...
![](https://i.ibb.co/Gscq9HB/overmodelout2.png)
