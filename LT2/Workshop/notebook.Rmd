...
If observations between samples (forming the different groups to be compared) are dependent (for example, if three measurements have been collected on the same individuals as it is often the case in developmental research), the repeated measures ANOVA should be preferred in order to take into account the dependency between the samples.
...
*3. These normal distributions have a common or equality of variance.*
...
```{r}
perfectionism <- read_csv("C:/Users/CURRANT/Dropbox/Work/LSE/PB130/LT2/Workshop/perfectionism.csv")
head(perfectionism)
```
...
The boxplots show a similar variance for the different countries. In the boxplot, this can be seen by the fact that the boxes and the whiskers have a comparable size. However, there are a couple of outliers as shown by the points outside the whiskers and the UK box seems a little fatter (this is likely becasue the sample size for UK is smaller so therefore more variance).

Like the normality assumption, if you feel that the visual approach is not sufficient, you can formally test for equality of the variances with a Levene’s test. I would suggest there is enough doubt in ur box plots to run this test.
...
1. For significance tests of models to be accurate, the sampling distribution of the thing you’re testing must be normal.Think back the indirect effect. Remember I showed you how the sampling distribution of the indirect effect is skewed? This is why we bootstrapped it. Bootstrapping is vital to overcome this assumption. 
...
*3. Homoscedasticity/homogeneity of variance* Homogeneity of variance occurs when the spread of scores for your dependednt variable is the same at each level of the predictor. Just as we have just looked at for ANOVA.

*4. Independence.* Just like ANOVA, independence is similarly important for regression. It means that the errors in your model are not related to each other (i.e., no clusting or repeated measures). Computation of standard error relies on the assumption of independence, so if you don’t have standard error, say goodbye to confidence intervals and significance tests. As I mentioned above, we will look more into this as we move to multi-level modelling. It is a design issue and cannot be tested usign stats.

*4. No influential outliers.* This isn’t technically an assumption of regression, but it’s best practice to avoid influential outliers. Why is it problematic to have outliers in your data? Outliers can bias parameter estimates (e.g., mean), and they also affect your sums of squares. Sums of squares are used to estimate the standard error, so if your sums of squares are biased, your standard error likely is too. 
It’s really bad to have a biased standard error because it is used to calculate confidence intervals around our parameter estimate. In other words, outliers could lead to biased confidence intervals. Not good! We need to rid ourselves of those pesky outliers. We looked at this topic in depth last semester when we cleaned the dataset. So make sure you always do a good job of cleaning and screening before you begin analyses!

## Testing The Linear ANOVA Assumptions
...
## Testing for linearity and homoscedasticty

To test for linearity and homoscedasticty we plot the fitted values (predicted values) against the residuals:
...
How did we do? R automatically flagged 3 data points that have large residuals (observations 192, 191 and 190). Besides that, our residuals appear to be pretty darn linear, as is evidenced by the pretty straight red line plotted through our residuals (if they were quadratic, for example, that line would resemble the shape of a parabola). It also looks like our data are homoscedastic, given that they appear evenly spread around the y = 0 line. Hooray!
...
```{r}
music.model <- lm(productivity ~ condition, data = music_data)
```
