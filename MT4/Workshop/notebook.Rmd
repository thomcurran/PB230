...
In this session, we are going to apply the data screening and cleaning strategies intorduced in the lecture.
...
So we are now left with a dataset in which missing caes have < 10% missing data.
...
Now we have imputed teh missing data and removed the outliers, we can now move to our analysis with the newly cleaned dataset. Let's run the liner model on it.
...
Now lets use MI to imputate the missing data. The following default settings are used in the mice function to start MI:

m=5 to generate 5 imputed datasets

maxit=10 to use 10 iterations for each imputed dataset

method=”pmm” to use predictive mean matching. For an elobate explanation of all options withing the mice function,type ?mice into the console.
