...
There are two main types of clustering: clutering within units and clustering within persons. let's begin by looking at clustering within units.
...
The first thing we do in multilevel modeling is check whether there is indeed within-cluster variance in the outcome variable. We do this with the intraclass correlation coefficient which is calculated using the total and residual variance from an empty model of happiness.
...
confint(friend.happy.fit, method="boot", nsim=100)
```
...
The blue line here shows the fixed intercept (i.e., where the fixed slope line crosses zero) and fixed slope of stress on time (i.e., the mean relationship over the sample). As we saw from the MLM output, the fixed slope is not significant even though it is a fairly large positive correlation. It is not significant, of course, because there is substantial variability between schools in relationships betwen friends and happiness as we can see in the individual slopes for each school.
...
At the end of the study, they took one measure of neuroticism. In this way, the researchers has data at two-levels. At level one, they had between person differences in neuroticism (i.e., how people differ from the sample mean in terms of their neuroticism). At level two, they had within-person differences in stress and negative affect (i.e., how people differ from their own mean stress and negative affect on a particular day).
...
Note the difference between this model and the school model is that we are interested in whether the level 1 stress trajectories are different depending on the level 2 variable: neuroticism. Therefore, we have a cross-level interaction in the model that tests this hyopthesis.
...
Like in the school example above, we often first fit an empty model that provides us information about how much of the total variance in the outcome variable is within-person variance and how much is between-person variance.
...
To partition the variance into between-person and within-person sources, we take the variance component of the intercept and divide it by the sum of the residual variance and the intercept variance. 
...
We now add trait neuroticism to the MLM model to see if and how between-person differences in neuroticism are related to the intercepts and trajectories of stress. Like in the example above, it is important to grand mean center the neuroticism variable so that the unit of measurement for neuroticism is deviation from the sample mean. To do this, we fit MLM model using the neurotic_gmc variable in the dataset.
...
2. day: As before, stress increased marginally over the course of the study days. For every unit increase in day, stress increased by .001 (95% CI = -.01, .02). This is not a significant change (i.e., CI includes zero). But as we saw above, we are not interested in the mean stress change, we are interested here in the variance of that mean change and whether it can be predicted by between-person differences in neuroticism
...
summary(neg.stress.model)

# Call bootstrap confidence intervals
confint(neg.stress.model, method="boot", nsim=100)
```
...
summary(neg.neurotic.model)

# Call bootstrap confidence intervals
confint(neg.neurotic.model, method="boot", nsim=100) 
```
...
```{r}
johnson_neyman(model=neg.neurotic.model, pred=stress_pmc, modx=neurotic_gmc)
```
